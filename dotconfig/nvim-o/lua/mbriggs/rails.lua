...
function M.find_models()
  create_finder("Models", "/app/models")
end

function M.find_controllers()
  create_finder("Controllers", "/app/controllers")
end

function M.find_views()
  create_finder("Views", "/app/views")
end

function M.find_helpers()
  create_finder("Helpers", "/app/helpers")
end

function M.find_mailers()
  create_finder("Mailers", "/app/mailers")
end

function M.find_jobs()
  create_finder("Jobs", "/app/jobs")
end

function M.find_lib()
  create_finder("Lib", "/lib")
end

function M.find_tests()
  create_finder("Tests", "/test")
end

function M.find_specs()
  create_finder("Specs", "/spec")
end
...
function M.find_related()
  ...
  -- Convert search patterns to rg glob format
  local args = {}
  ...
  require("snacks").picker.files({
    prompt = display_full_name .. "❯ ",
    cwd = rails_root,
    cmd = "rg",
    args = vim.list_extend(
      { "--files", "--no-messages", "--color", "never" },
      args
    ),
    confirm = function(picker, selected)
      if selected and selected.file ~= current_file then
